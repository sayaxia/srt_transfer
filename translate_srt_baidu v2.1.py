...
class SRTTranslator:
    ...
    def translate_srt_file(self, input_file: str, output_file: str, target_lang: str = "zh") -> None:
        ...
        def log_translation(index, text):
            ...
            return f"{text}\n{translated}"  # 双语字幕：原文 + 翻译

        with ThreadPoolExecutor(max_workers=10) as executor:  # QPS=10
            translated_texts = list(executor.map(lambda args: log_translation(*args), enumerate(texts)))

        # 确保字幕内容正确更新
        if len(translated_texts) != len(subtitles):
            print(f"错误：翻译后的字幕数量与原始字幕数量不一致！原字幕数量：{len(subtitles)}，翻译结果数量：{len(translated_texts)}")
            return

        for sub, translated in zip(subtitles, translated_texts):
            sub.content = translated  # 更新为双语字幕
...
def main() -> None:
    ...
    # 检测是否通过拖拽文件运行
    if len(sys.argv) > 1:
        input_file = sys.argv[1]  # 获取拖拽的文件路径
    else:
        input_file = select_file()  # 弹窗选择文件
